...
- `SegmentedMetricBlock` for fixed-count progress rows, ordered funnel columns, balanced capsule rings, part-to-whole strips, and distribution rows using one generic item model and a `SegmentedMetricStyle` visual-treatment enum.
...
Use `CapsuleRing` for intentionally balanced closed-loop compositions. Skewed inventory and count data should use `CompositionStrip` or `DistributionRows` so exact values stay calm and readable.
