...
- Node kinds can be styled in one reusable pass with `.WithNodesOfKind(TopologyNodeKind.Certificate, color: "#2563EB", backgroundColor: "#EFF6FF")`, which is useful for relationship cards where certificates, owners, endpoints, services, organizations, or other host-mapped concepts need stable visual identity across several topology diagrams.
...
- Legend node entries can use `AddNodeKind(..., symbol: "TLS", backgroundColor: "#EFF6FF")`, and legend edge entries can use `AddEdgeKind(..., lineStyle: TopologyEdgeLineStyle.Dotted)`, so "verified", "observed", and "inferred" legend keys match the actual markers and route styles instead of always showing generic symbols.
- Auto and merged legends infer shared node-kind accent/background colors, edge-kind colors, and edge-kind line styles when every item in that inferred legend bucket uses the same explicit styling, so reusable calls such as `.WithNodesOfKind(...)` and `.WithEdgesOfKind(...)` keep the legend aligned without hand-building per-chart legend entries.
