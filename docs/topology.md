...
`Overview` contains a summary node for each page and counts relationships between pages. `Source` retains the complete prepared topology and its semantics. Each detail page is also a `PreparedTopology`, so callers can export it or pass its envelope to native-document adapters without laying it out again.
...
The HTML adapter renders a complete navigator with `report.ToInteractiveHtmlPage()` from `ChartForgeX.Interactivity.Html`. It mounts one SVG at a time, searches node names and stable ids with at most 20 visible results, and links cross-page relationships to their destination pages. Natural-size scrolling is the default; readers can choose Fit page when an overview matters more than label size. Export SVG or PNG separately for readers without JavaScript.
