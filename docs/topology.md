...
Use `.AddHierarchy(...)` when the source data is naturally parent-child, such as forest -> domain -> group -> user, tenant -> subscription -> resource group -> service, team -> manager -> member, or network core -> distribution -> access. Each `TopologyHierarchyItem` can provide an explicit `Level`, or ChartForgeX can infer levels from `ParentId`. `TopologyHierarchyOptions.MinLevel` and `MaxLevel` let a host render levels `0`, `0..1`, `0..2`, `0..3`, or focused windows such as `2..3` from the same source data without rebuilding the model. When `IncludeAncestorContext` is enabled, ancestors below `MinLevel` remain visible as breadcrumb context; disable it for exact level-only slices. Generated nodes receive `Metadata["layer"]`, `Metadata["hierarchy.level"]`, `Metadata["hierarchy.context"]`, `Metadata["hierarchy.layoutPolicy"]`, and optional `Metadata["hierarchy.parentId"]`, then the chart defaults to layered layout. These builder-owned metadata keys remain authoritative when caller metadata is copied. The layered layout centers child buckets beneath their visible parents, routes the first sibling band through a shared bus, and uses one obstacle-checked exterior trunk for later compact or vertical bands so connectors do not cross earlier node cards.

Set `TopologyHierarchyItem.LayoutPolicy` or `TopologyTeamMember.LayoutPolicy` when one branch needs a different sibling arrangement. The policy inherits through that subtree until another item overrides it. `Auto` keeps direct children in one standard sibling band when it fits and resolves crowded branches to a balanced compact grid. `Standard` always keeps one sibling band perpendicular to the chart flow, `Compact` always uses balanced rows and columns, and `Vertical` stacks direct children in one page-relative column. Consequently, `Standard` and `Vertical` have the same direct-child geometry in a left-to-right or right-to-left chart. `TopologyHierarchyOptions.LayoutPolicy` and `TopologyTeamOptions.LayoutPolicy` set the default for the complete input hierarchy. Prepared nodes expose the resolved geometry through `Metadata["layout.hierarchyPolicy"]`, `layout.row`, and `layout.column`; SVG includes the configured metadata when `IncludeDataAttributes` is enabled.
...
- `Layered` uses node kind or `Metadata["layer"]` for top-to-bottom, bottom-to-top, left-to-right, or right-to-left layers. Hierarchy levels with visible parents are bucketed near those parents and honor inherited `Auto`, `Standard`, `Compact`, or `Vertical` subtree policies before falling back to the generic grid. Multi-child hierarchy fan-outs receive shared connector bus waypoints and expose `hierarchy.route = shared-bus` on generated edges. Wrapped fan-outs also expose `hierarchy.route.tier` plus `hierarchy.route.busY` or `hierarchy.route.busX`, so dashboards can inspect row/column bus lanes that avoid cutting through child cards. Prepared nodes expose `layout.layer`, `layout.layerIndex`, `layout.row`, `layout.column`, and `layout.hierarchyPolicy` metadata for host diagnostics.
...
- Set `TopologyRenderOptions.ReadableDenseLayout = true` for large replication-style `DenseGrouped` maps (off by default, so existing charts render unchanged). With it, left-to-right and right-to-left layouts with more than six groups wrap site panels into rows no wider than the viewport content width, or the widest panel when that is wider (declared group order is kept, 44 px gutters between panels; the viewport grows downward and the legend stays below the rows). With the `Auto` policy, groups of 10–24 nodes use a card `Grid` and only groups above 24 nodes collapse to dots. `Grid` and `PairRows` spacing reserves room for tile captions plus a routing gutter. Obstacle-avoiding routes treat tile captions as part of the card and fall back to a grid search (A* over the card and header edges) when the best corridor route would cut through another card or its caption, so replication fixtures of 76–144 DCs route with zero card crossings (see `ChartForgeX.Tests/TopologyReplicationBudgetTests.cs`). Hosts should still aggregate to site level above roughly 40 DCs. Without the option, `Auto` groups of ten or more nodes still collapse to dots, panels stay in one row or grid, and routes ignore captions as before.
...
### Advanced edge contracts and diagnostics

Use `AddNodePort(...)` and `WithEdgeNamedPorts(...)` when a route must attach to a stable named endpoint rather than an inferred side. `WithEdgeStroke(...)` controls per-edge width, opacity, and arbitrary even-length dash patterns. `WithEdgeMarkers(...)` independently selects the source and target marker, while `WithEdgeEndpointLabels(...)` places short protocol or port labels beside the endpoints. `WithEdgeLayoutHints(...)` supplies preferred force-layout length, minimum layered rank span, and routing priority; these are deterministic layout hints, not a promise that every constrained route has an exact physical length.

Card nodes can use `AddNodeDetail(...)` for typed label/value rows with optional icon, status, and color. `TopologyLayoutPreset.Dense`, `Compact`, `Balanced`, and `Presentation` apply reusable spacing profiles. For authoring tools and CI, `TopologyLayoutDiagnostics.Analyze(...)` returns prepared viewport, group, node, named-port, route, obstacle, and collision geometry. `IncludeLayoutDiagnosticOverlay` draws those bounds and route points in SVG/PNG for debugging and should remain disabled in normal report output.
