...
- Auto and merged legends infer shared edge-kind colors and line styles when every edge of that kind uses the same explicit styling, so reusable calls such as `.WithEdgesOfKind(...)` keep the legend aligned without hand-building per-chart legend entries.
