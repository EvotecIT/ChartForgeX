...
The planner leaves the source scene unchanged and examines the full source graph. Replan after changing its membership or relationships. Static neighborhoods with no authored coordinates or routes receive a deterministic radial layout; authored geometry is preserved. Interactive Focus uses the same default budgets: 13 visible nodes and 24 relationships. Configure them through `graph.Options.Neighborhood`. The browser respects active filters and collapsed or hierarchical views when choosing eligible objects; hidden objects can still connect the discovery path.

Select a node and choose **Focus**. **Previous neighbors** and **Next neighbors** page through the neighborhood; selecting a visible node drills into its neighborhood. **Back** restores the previous page and viewport, and **Overview** restores the entry viewport and selection. Focus pauses physics, preserves node coordinates and pins, and leaves physics paused until explicitly restarted. Authored dense positions remain dense; focusing does not rearrange them. A filter that removes every neighbor from the current page resets paging to the first eligible page.

Hosts can call `ChartForgeXGraphExplorer.focus(target, nodeId, { hops: 2, maximumNodes: 13, maximumEdges: 24 })`, `focusPage(target, 1)`, `focusBack(target)`, or `focus(target, null)` to return to the overview. Missing or unavailable roots return `false`; invalid numeric budgets throw `RangeError`. State snapshots include the focus budgets, page offset, and bounded navigation history. `cfxgraphfocus` reports visible, omitted, scope, and boundary counts.
