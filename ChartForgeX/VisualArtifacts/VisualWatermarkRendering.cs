...
internal static class VisualWatermarkRendering {
    ...
    public static string ApplyToSvg(string svg, VisualArtifact artifact, IReadOnlyList<VisualWatermark> watermarks) {
        ...
        for (var i = 0; i < watermarks.Count; i++) AppendSvgWatermark(layer, watermarks[i], size.Width, size.Height, i);
        layer.Append("</g>");
    ...
    private static void AppendSvgWatermark(StringBuilder output, VisualWatermark watermark, double width, double height, int index) {
        if (watermark.Repeat) {
            ...
            for (var y = watermark.RepeatSpacingY / 2; y < height; y += watermark.RepeatSpacingY) {
                ...
                for (var x = watermark.RepeatSpacingX / 2 - stagger; x < width; x += watermark.RepeatSpacingX) {
                    AppendSvgWatermarkAt(output, watermark, x, y, index);
                }
            }
            return;
        }
        ...
        AppendSvgWatermarkAt(output, watermark, bounds.CenterX, bounds.CenterY, index, bounds.Width, bounds.Height);
    }

    private static void AppendSvgWatermarkAt(StringBuilder output, VisualWatermark watermark, double centerX, double centerY, int index, double? explicitWidth = null, double? explicitHeight = null) {
        ...
        output.Append("<image data-cfx-role=\"watermark\" data-cfx-watermark-index=\"").Append(index).Append("\"")
            ...
            .Append(" href=\"data:").Append(watermark.ImageMimeType).Append(";base64,").Append(Convert.ToBase64String(bytes)).Append("\"");
        if (Math.Abs(watermark.RotationDegrees) > 0.0001) {
            output.Append(" transform=\"rotate(").Append(F(watermark.RotationDegrees)).Append(' ').Append(F(centerX)).Append(' ').Append(F(centerY)).Append(")\"");
        }
        output.Append(" />");
    ...
    private static void DrawRasterWatermark(RgbaCanvas canvas, VisualWatermark watermark) {
        if (watermark.Repeat) {
            ...
            for (var y = watermark.RepeatSpacingY / 2; y < canvas.Height; y += watermark.RepeatSpacingY) {
                var stagger = row++ % 2 == 0 ? 0 : watermark.RepeatSpacingX / 2;
                for (var x = watermark.RepeatSpacingX / 2 - stagger; x < canvas.Width; x += watermark.RepeatSpacingX) DrawRasterWatermarkAt(canvas, watermark, x, y);
            }
            return;
        }
    ...
    private static void DrawRasterWatermarkAt(RgbaCanvas canvas, VisualWatermark watermark, double centerX, double centerY, double? explicitWidth = null, double? explicitHeight = null) {
        ...
        var source = RasterImageDecoder.Decode(watermark.ImageBytes!);
        ...
        var prepared = ScaleAndRotate(source, targetWidth, targetHeight, watermark.RotationDegrees, watermark.Opacity);
    ...
    private static WatermarkBounds ResolveBounds(VisualWatermark watermark, double canvasWidth, double canvasHeight) {
        double width;
        double height;
        if (watermark.Kind == VisualWatermarkKind.Text) {
            var fontSize = watermark.FontSize * watermark.Scale;
