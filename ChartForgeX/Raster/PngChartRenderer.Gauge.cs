...
public sealed partial class PngChartRenderer {
    private static void DrawGauge(RgbaCanvas c, Chart chart, ChartRect plot) {
        ...
        c.DrawTextEmphasized(cx - EstimatePngEmphasizedTextWidth(label, valueFontSize) / 2.0, cy - radius * 0.1 - valueFontSize / 2.0, label, theme.Text, valueFontSize);
        c.DrawTextEmphasized(cx - EstimatePngEmphasizedTextWidth(gauge.Name, nameFontSize) / 2.0, cy + 34 - nameFontSize + 1, gauge.Name, theme.MutedText, nameFontSize);
        c.DrawCircle(cx - EstimatePngEmphasizedTextWidth(statusLabel, tickFontSize) / 2.0 - 9, cy + 53, 5.2, theme.CardBackground);
        c.DrawCircle(cx - EstimatePngEmphasizedTextWidth(statusLabel, tickFontSize) / 2.0 - 9, cy + 53, 2.5, statusColor);
        c.DrawTextEmphasized(cx - EstimatePngEmphasizedTextWidth(statusLabel, tickFontSize) / 2.0, cy + 57 - tickFontSize + 1, statusLabel, theme.MutedText, tickFontSize);
