...
public sealed partial class SvgVisualBlockRenderer {
    private static void RenderFishboneDiagram(SvgMarkupWriter writer, FishboneDiagramBlock block) {
        ...
        WriteText(writer, block.Effect, layout.Head.X + 10, layout.Head.Y + layout.Head.Height * 0.55, layout.Head.Width - 20, TextAlignment.Center, theme.Text, theme.FontFamily, Math.Max(11, theme.SubtitleFontSize + 1), "800");
    }
    ...
    private static void WriteFishboneBranch(SvgMarkupWriter writer, VisualBlockRendering.FishboneBranchPlacement branch, ChartForgeX.Themes.ChartTheme theme) {
        ...
        WriteText(writer, branch.Cause.Label, branch.LabelX, branch.LabelY, branch.LabelWidth, TextAlignment.Center, theme.Text, theme.FontFamily, Math.Max(10, theme.SubtitleFontSize), "750");
        for (var index = 0; index < branch.Children.Count; index++) {
            ...
            WriteText(writer, child.Cause.Label, child.LabelX, child.LabelY, child.LabelWidth, TextAlignment.Right, theme.MutedText, theme.FontFamily, Math.Max(8.5, theme.SubtitleFontSize - child.Depth), "600");
        }
    }
}
