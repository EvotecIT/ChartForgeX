...
internal static partial class SmokeTests {
    private static void GraphExplorerEditingStateAndBoxSelectionStayOptIn() {
        ...
        Assert(html.Contains("const persisted = { ...state, document: null }", StringComparison.Ordinal) && html.Contains("options?.source !== 'storage'", StringComparison.Ordinal), "Persistent interaction state should retain presentation choices without replacing fresh graph content, including when an older stored snapshot still contains a document.");
        Assert(html.Contains("focusedItem && root.contains(focusedItem)", StringComparison.Ordinal) && html.Contains("clusterFallback || memberFallback || active || scene", StringComparison.Ordinal), "Expanding or collapsing a cluster should transfer keyboard focus within that aggregate before hidden graph items leave the accessibility tree.");
        Assert(html.Contains("const graphItemAccessible", StringComparison.Ordinal) && html.Contains("graphItemAccessible(root, candidate)", StringComparison.Ordinal), "Keyboard navigation should exclude relationships whose endpoints are hidden inside collapsed clusters.");
        Assert(html.Contains("event.defaultPrevented) return", StringComparison.Ordinal) && html.Contains("}, true);", StringComparison.Ordinal), "Hierarchy drill keys should run before generic spatial keyboard navigation.");
        ...
        Assert(html.Contains("!Object.prototype.hasOwnProperty.call(metadata, 'topology.backgroundColor')", StringComparison.Ordinal) && html.Contains("node.style.setProperty('--cfx-node-fill', palette.card)", StringComparison.Ordinal) && html.Contains("node.firstElementChild?.style.setProperty('--cfx-node-fill', palette.card)", StringComparison.Ordinal), "Theme switching should recolor the card container and its physical SVG mark while preserving caller-provided backgrounds.");
        ...
        Assert(style.Contains("fill: var(--cfx-node-stroke, var(--cfx-color-graph-text)) !important", StringComparison.Ordinal), "Card icons should retain visible contrast on light and dark card surfaces.");
