...
internal static partial class SmokeTests {
    private static void GraphSceneContractsModelExplorerPhysicsAndLod() {
        ...
        scene.Options.Physics.Stabilization.Iterations = 700;
        ...
        Assert(scene.Options.Physics.Solver == GraphPhysicsSolver.BarnesHut && scene.Options.Physics.Stabilization.Iterations == 700, "Graph scenes should carry solver profiles without binding the core contract to a browser engine.");
        ...
        invalidPhysics.Options.Physics.BarnesHut.Damping = double.NaN;
        ...
        var zeroRepulsion = GraphScene.Create("zero-repulsion", "Zero repulsion").AddNode("node", "Node"); zeroRepulsion.Options.Physics.Repulsion.Strength = 0;
    ...
    private static void GraphExplorerHtmlAdapterRendersSelfContainedScene() {
        ...
        scene.Options.Physics.Stabilization.Iterations = 900;
        ...
        Assert(html.Contains("data-cfx-physics-stabilization-iterations=\"900\"", StringComparison.Ordinal), "Graph explorer pages should expose stabilization budgets.");
        Assert(html.Contains("data-cfx-physics-force-atlas2-gravitational-constant=\"-50\"", StringComparison.Ordinal) && html.Contains("data-cfx-physics-force-atlas2-damping=\"0.4\"", StringComparison.Ordinal), "Graph explorer pages should expose solver-specific force simulation constants.");
        ...
        Assert(html.Contains("aria-label=\"API\"", StringComparison.Ordinal) && html.Contains("aria-label=\"Core services, 2 objects", StringComparison.Ordinal), "Graph explorer focusable SVG nodes and collapsed cluster summaries should expose accessible names and cluster cardinality.");
        ...
        var edgeRendererSource = System.IO.File.ReadAllText(System.IO.Path.Combine(FindRepositoryRoot(), "ChartForgeX.Interactivity.Html", "HtmlGraphExplorerRenderer.Edges.cs"));
        ...
        Assert(rendererSource.Contains("BuildCollapsedNodeRenderRadii", StringComparison.Ordinal) && edgeRendererSource.Contains("collapsedNodeRadii.TryGetValue(edge.TargetNodeId", StringComparison.Ordinal) && edgeRendererSource.Contains("collapsedNodeRadii.TryGetValue(edge.SourceNodeId", StringComparison.Ordinal), "Graph explorer static collapsed-cluster arrows should trim against each cluster summary radius instead of a fixed inset.");
        ...
        Assert(html.Contains("data-cfx-role=\"graph-cluster\" tabindex=\"-1\" aria-hidden=\"false\"", StringComparison.Ordinal) && html.Contains("const active = focusableItems.find", StringComparison.Ordinal), "Graph explorer SVG should use a runtime roving tab stop instead of adding every collapsed cluster to the page tab order.");
        Assert(html.Contains("class=\"cfx-graph-node cfx-graph-cluster-collapsed-member\" tabindex=\"-1\" data-cfx-role=\"graph-node\" role=\"button\" aria-pressed=\"false\" data-node-id=\"api\"", StringComparison.Ordinal) && html.Contains("class=\"cfx-graph-edge cfx-graph-cluster-collapsed-member\" tabindex=\"-1\" data-cfx-role=\"graph-edge\" role=\"button\" aria-pressed=\"false\" data-edge-id=\"api-db\"", StringComparison.Ordinal) && html.Contains("class=\"cfx-graph-edge-label cfx-graph-cluster-collapsed-member\" data-cfx-role=\"graph-edge-label\" data-edge-label-for=\"api-db\"", StringComparison.Ordinal), "Graph explorer SVG should render collapsed cluster members as named selection controls outside the tab order before browser bindings run.");
        ...
        Assert(html.Contains("const uniformScale = renderer === 'svg' ? Math.min(rect.width / size.width, rect.height / size.height) : 0", StringComparison.Ordinal) && html.Contains("const offsetY = (rect.height - contentHeight) / 2", StringComparison.Ordinal), "Graph explorer SVG pointer mapping should account for centered letterboxing in narrow touch layouts.");
        ...
        Assert(html.Contains("const cadenceBudget = budget + Math.max(1, budget * .1)", StringComparison.Ordinal) && html.Contains("renderWorkMs > budget || frameMs > cadenceBudget", StringComparison.Ordinal), "Graph explorer frame telemetry should distinguish measured render work from normal display cadence jitter.");
        ...
        Assert(html.Contains("toggleNeighborhoodFocus", StringComparison.Ordinal) && html.Contains("cfxgraphfocus", StringComparison.Ordinal) && html.Contains("cfxGraphFocusNode", StringComparison.Ordinal) && html.Contains(".cfx-graph-neighborhood-primary ellipse", StringComparison.Ordinal), "Graph explorer runtime should expose selected-node neighborhood focus, host events, and rich-shape SVG focus styling.");
        ...
        Assert(html.Contains("drawCanvas(root, state, { force: true, exportDisclosure: true })", StringComparison.Ordinal), "Graph explorer PNG export should paint the Canvas surface and disclose priority-only views even when the active renderer is SVG.");
        ...
        Assert(html.Contains("applyFilters(root); syncBundledEdgePresentation(root, state); updateEdges(root, state.edges);", StringComparison.Ordinal), "Graph explorer SVG edges and cached Canvas labels should refresh after cluster collapse or expansion toggles.");
        ...
        Assert(html.Contains("cfx-graph-edge-label.cfx-graph-label-selected", StringComparison.Ordinal) && html.Contains("cfx-graph-edge-label.cfx-graph-neighborhood-related", StringComparison.Ordinal) && html.Contains("display: block;", StringComparison.Ordinal), "Graph explorer LOD should reveal selected and focused edge labels even when dense graphs hide ordinary labels.");
        Assert(html.Contains("(!root.classList.contains('cfx-graph-lod-hide-edge-labels') || selected || related)", StringComparison.Ordinal) && html.Contains("(!root.classList.contains('cfx-graph-priority-overview') || selected || related)", StringComparison.Ordinal), "Graph explorer Canvas LOD and priority overviews should reveal selected and focused labels without crowding the default view.");
        Assert(html.Contains("syncSelectedEdgeLabels", StringComparison.Ordinal) && html.Contains("...state.clusters.filter(cluster => graphItemAccessible(root, cluster.el))", StringComparison.Ordinal) && html.Contains("...state.edges.filter(edge => graphItemAccessible(root, edge.el) && num(edge.el, 'data-cfx-bundle-count', 0) > 1)", StringComparison.Ordinal), "Accelerated keyboard selection should include accessible collapsed clusters and bundled routes as well as nodes.");
        ...
        Assert(html.Contains("kind: attr(edge, 'data-edge-kind')", StringComparison.Ordinal) && html.Contains("showLabel: attr(edge, 'data-edge-show-label') !== 'false'", StringComparison.Ordinal) && html.Contains("hidden: attr(edge, 'data-edge-hidden') === 'true'", StringComparison.Ordinal), "Graph explorer JSON export should include edge facets and intrinsic visibility needed to reconstruct reviewed graphs without persisting transient view state.");
        ...
        Assert(html.Contains("routePoints: routePoints(attr(el, 'data-edge-route-points'))", StringComparison.Ordinal) && html.Contains("routeRenderPoints(edge).map", StringComparison.Ordinal) && html.Contains("endpoint.x + node.x - homeX", StringComparison.Ordinal) && html.Contains("distanceToRoute(point, routeRenderPoints(rendered))", StringComparison.Ordinal), "Graph explorer SVG, Canvas/PNG, export, and hit-testing paths should consume prepared edge route points while keeping untrimmed route endpoints attached to live node positions.");
        Assert(html.Contains("dragThreshold", StringComparison.Ordinal) && html.Contains("if (!active.moved) {", StringComparison.Ordinal) && html.Contains("node.fixed = release ? active.movingFixed : true", StringComparison.Ordinal), "Graph explorer node selection should only start a drag after pointer movement and should apply the configured release policy.");
        Assert(html.Contains("const hitItem = node || overlayHit || (runtimeOverlay ? hitGraphItemAt(root, point) : graphItem) || hitGraphItemAt(root, point)", StringComparison.Ordinal) && html.Contains("hasFeature(root, 'Viewport') && !hitBlocksPan", StringComparison.Ordinal), "Graph explorer viewport panning should start from graph background or inert graph items after accelerated overlays resolve to their virtual graph items.");
        ...
        Assert(html.Contains("physicsProfileAttribute", StringComparison.Ordinal) && html.Contains("data-cfx-physics-barnes-hut", StringComparison.Ordinal), "Graph explorer physics should resolve solver-specific tuning rather than approximate solver names with global multipliers.");
        ...
        Assert(html.Contains("containLayoutOutliers", StringComparison.Ordinal) && html.Contains("cfxGraphLayoutOutliersContained", StringComparison.Ordinal), "Large stabilized graphs should contain sparse leaf outliers before fitting so the primary topology does not collapse into one corner.");
        Assert(html.Contains("runLayoutQualityPass", StringComparison.Ordinal) && html.Contains("spreadHubNeighborhoods", StringComparison.Ordinal) && html.Contains("separateOverlaps", StringComparison.Ordinal) && html.Contains("Math.max(nodeHalfWidth(node), nodeHalfHeight(node))", StringComparison.Ordinal), "Graph explorer runtime physics should run a structured shape-aware quality pass after stabilization instead of only fitting whatever the force solver produced.");
        ...
        Assert(html.Contains("if (rawDistance < 0.001)", StringComparison.Ordinal)
               && html.Contains("* Math.PI / 180", StringComparison.Ordinal),
            "Graph explorer pairwise physics should force a non-zero deterministic angular jitter vector for perfectly overlapping node pairs.");
        ...
        Assert(html.Contains("root.__cfxGraphState ||", StringComparison.Ordinal) && html.Contains("(renderer === 'canvas' || renderer === 'webgl') && currentState", StringComparison.Ordinal), "Graph explorer viewport updates should reuse cached graph state and redraw only the active accelerated surface while SVG is active.");
        ...
        Assert(html.Contains("root.__cfxGraphPointerSelectionId = node.id", StringComparison.Ordinal)
               && html.Contains("root.__cfxGraphSuppressClickId = node.id", StringComparison.Ordinal),
            "Graph explorer click suppression should consume the follow-up click for pointer-selected nodes even after slow modifier clicks.");
        ...
        Assert(html.Contains(".cfx-graph-has-overview:not(.cfx-graph-overview-unneeded) .cfx-graph-overview", StringComparison.Ordinal) && html.Contains("display: block;", StringComparison.Ordinal), "Graph explorer CSS should show the overview only when viewport navigation is enabled and the visible graph is large enough to benefit from it.");
        Assert(html.Contains(".cfx-graph-edge.cfx-graph-selected", StringComparison.Ordinal) && html.Contains("stroke: var(--cfx-edge-stroke, var(--cfx-color-edge))", StringComparison.Ordinal), "Graph explorer CSS should visibly preserve selected edge state even when edges carry custom color variables.");
        Assert(html.Contains(".cfx-graph-node.cfx-graph-selected", StringComparison.Ordinal) && html.Contains("stroke: var(--cfx-node-stroke", StringComparison.Ordinal), "Graph explorer CSS should visibly preserve selected node state even when nodes carry custom style variables.");
        ...
        Assert(html.Contains(".cfx-graph-stage", StringComparison.Ordinal) && html.Contains(".cfx-graph-svg", StringComparison.Ordinal) && html.Split(new[] { "aspect-ratio: 12 / 7" }, StringSplitOptions.None).Length >= 3 && !html.Contains("min-height:420px", StringComparison.Ordinal), "Graph explorer CSS should preserve one SVG scene aspect ratio across the stage and SVG surface so pointer mapping does not drift under narrow embeds.");
        Assert(!html.Contains("rel=\"stylesheet\"", StringComparison.OrdinalIgnoreCase), "Graph explorer pages should not reference external stylesheets.");
        ...
        Assert(HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains(".cfx-graph-explorer *", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains("box-sizing: inherit", StringComparison.Ordinal), "Host-registered graph explorer CSS should scope base box sizing to fragments without requiring a page-level shell.");
        Assert(HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains(".cfx-graph-explorer {", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains("position: relative", StringComparison.Ordinal), "Host-registered graph explorer CSS should anchor graph tooltips to the explorer component.");
        Assert(HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains("touch-action: auto", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains(".cfx-graph-interactive-touch .cfx-graph-stage", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildFragmentStyle().Contains("touch-action: none", StringComparison.Ordinal), "Graph explorer CSS should preserve native page scrolling for static embeds and disable touch scrolling only when the graph consumes touch movement.");
        ...
        Assert(HtmlGraphExplorerRenderer.BuildInteractionScript().Contains("reheatPhysics(root, 'drag-start'", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildInteractionScript().Contains("releaseDraggedPhysicsNode(root, node)", StringComparison.Ordinal), "Graph explorer runtime should reheat connected nodes while dragging and release the node back into the simulation.");
        ...
        Assert(webGlHtml.Contains("data-cfx-graph-renderer=\"webgl\"", StringComparison.Ordinal) && webGlHtml.Contains("data-cfx-role=\"graph-webgl\"", StringComparison.Ordinal), "Graph explorer should preserve explicit WebGL requests and render the dedicated accelerated surface.");
        ...
        Assert(HtmlGraphExplorerRenderer.BuildInteractionScript().Contains("configured === 'webgl' && webGlAvailable(root) ? 'webgl'", StringComparison.Ordinal) && HtmlGraphExplorerRenderer.BuildInteractionScript().Contains("root.dataset.cfxGraphRendererFallback", StringComparison.Ordinal), "Graph explorer runtime should activate WebGL when available and report the Canvas fallback when acceleration is unavailable.");
        ...
        var nodeSideClusterHtml = GraphScene.Create("node-side-cluster", "Node side cluster")
            ...
            .ToGraphExplorerHtmlFragment();
        Assert(nodeSideClusterHtml.Contains("data-cluster-node-ids=\"a\"", StringComparison.Ordinal) && nodeSideClusterHtml.Contains("class=\"cfx-graph-node cfx-graph-cluster-collapsed-member\" tabindex=\"-1\" data-cfx-role=\"graph-node\" role=\"button\" aria-pressed=\"false\" data-node-id=\"a\"", StringComparison.Ordinal), "Graph explorer collapsed clusters should honor node-side ClusterId membership even when GraphSceneCluster.NodeIds is not duplicated.");
        Assert(!nodeSideClusterHtml.Contains("class=\"cfx-graph-edge cfx-graph-cluster-collapsed-member\"", StringComparison.Ordinal), "Graph explorer collapsed clusters should keep cross-boundary edges visible for aggregate rendering.");
        ...
        Assert(!loadCollapsedCrossEdgeMarkup.Contains("class=\"cfx-graph-edge cfx-graph-cluster-collapsed-member\"", StringComparison.Ordinal), "Graph explorer static load-collapse should preserve cross-cluster edges for aggregate rendering.");
        ...
        AssertRoutedLabelCollisionStaysOnPolyline();
        var boundaryRouteHtml = GraphScene.Create("boundary-routed-arrow", "Boundary routed arrow").AddNode("source", "Source", node => { node.X = 100; node.Y = 260; }).AddNode("target", "Target", node => { node.X = 520; node.Y = 260; }).AddEdge("route", "source", "target", "Route", edge => { edge.Shape = GraphEdgeShape.Polyline; edge.RoutePoints.AddRange(new[] { new GraphScenePoint(100, 245), new GraphScenePoint(100, 120), new GraphScenePoint(520, 120), new GraphScenePoint(520, 245) }); edge.Directed = true; edge.SourceArrow = true; }).ToGraphExplorerHtmlFragment();
        Assert(ExtractGraphEdgePath(boundaryRouteHtml, "route").Contains("M 100 245", StringComparison.Ordinal) && ExtractGraphEdgePath(boundaryRouteHtml, "route").Contains("L 520 245", StringComparison.Ordinal), "Graph explorer SVG should not double-trim prepared route endpoints that already land on node boundaries.");
        ...
        var interactionScript = HtmlGraphExplorerRenderer.BuildInteractionScript();
        ...
        Assert(scaleHtml.Contains("data-cfx-graph-accelerated-markup=\"true\"", StringComparison.Ordinal) && scaleHtml.Contains("data-cfx-role=\"graph-document\"", StringComparison.Ordinal) && interactionScript.Contains("ensureGraphDocument", StringComparison.Ordinal) && interactionScript.Contains("materializeAcceleratedSvg", StringComparison.Ordinal) && interactionScript.Contains("drawAcceleratedSvgRuntime", StringComparison.Ordinal), "Accelerated large scenes should use a compact graph document while materializing premium SVG artwork for exports and small hierarchy views.");
        Assert(interactionScript.Contains("cfx-graph-node-image-rect", StringComparison.Ordinal) && interactionScript.Contains("appendExportedEdgeLabel", StringComparison.Ordinal), "Accelerated SVG materialization should preserve rectangular image crops and visible relationship labels.");
        Assert(interactionScript.Contains("nodePolygonPoints(node.shape, size)", StringComparison.Ordinal) && interactionScript.Contains("nodeDatabasePath(size)", StringComparison.Ordinal), "Accelerated SVG materialization should preserve star, diamond, triangle, and database marks through shared graph geometry helpers.");
        Assert(interactionScript.Contains("clone.style.setProperty(name, computed.getPropertyValue(name))", StringComparison.Ordinal), "Standalone SVG downloads should carry the resolved graph theme variable scope.");
        Assert(interactionScript.Contains("state.nodes.filter(node => visible(node.el)).length", StringComparison.Ordinal) && interactionScript.Contains("visibleNodeCount === '' ? totalNodeCount", StringComparison.Ordinal) && interactionScript.Contains("fullState.nodes.filter(node => visible(node.el))", StringComparison.Ordinal), "LOD, performance gates, and physics should scale against the visible hierarchy stage while retaining the full runtime scene.");
        ...
        var scaleBounds = GraphDocumentNodeBounds(scaleHtml, 1000);
