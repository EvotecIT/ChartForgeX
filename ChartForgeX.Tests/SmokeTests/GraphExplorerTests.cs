...
internal static partial class SmokeTests {
    ...
    private static void GraphExplorerHtmlAdapterRendersSelfContainedScene() {
        ...
        Assert(html.Contains("routePoints: routePoints(attr(el, 'data-edge-route-points'))", StringComparison.Ordinal) && html.Contains("routeRenderPoints(edge).map", StringComparison.Ordinal) && html.Contains("if (!trim) return endpoint", StringComparison.Ordinal) && html.Contains("distanceToRoute(point, routeRenderPoints(rendered))", StringComparison.Ordinal), "Graph explorer SVG, Canvas/PNG, export, and hit-testing paths should consume prepared edge route points while re-anchoring endpoints to live node positions only when trimming is required.");
