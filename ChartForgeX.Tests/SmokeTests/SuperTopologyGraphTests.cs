...
internal static partial class SmokeTests {
    ...
    private static void TopologyGraphExplorerScalesFromCardsToClusteredOverviews() {
        ...
        Assert(mediumHtml.Contains("data-cluster-node-count=\"8\"", StringComparison.Ordinal) && mediumHtml.Contains(">8 objects</text>", StringComparison.Ordinal) && mediumHtml.Contains("if (!visible(item)) return false", StringComparison.Ordinal) && mediumHtml.Contains("graphItemAccessible(root, item)", StringComparison.Ordinal), "Collapsed topology groups should show their object counts and keep visible cross-site routes keyboard reachable while hidden member relationships remain inaccessible.");
