...
internal static partial class SmokeTests {
    ...
    private static void GraphExplorerRuntimePatchesPreserveVisualAndReferenceContracts() {
        ...
        var validation = File.ReadAllText(Path.Combine(assetRoot, "graph-explorer.39-patch-validation.js"));
        var hierarchy = File.ReadAllText(Path.Combine(assetRoot, "graph-explorer.13-hierarchy.js"));
        Assert(validation.Contains("edges.forEach((edge, edgeId)", StringComparison.Ordinal) && validation.Contains("references a missing endpoint", StringComparison.Ordinal), "Browser graph patches should validate every surviving edge against the final node set before mutating the document.");
        ...
        Assert(api.Contains("syncGraphPatchClusterMembership", StringComparison.Ordinal) && validation.Contains("data-source-cluster-id", StringComparison.Ordinal) && validation.Contains("data-target-cluster-id", StringComparison.Ordinal), "Browser graph patches should synchronize node moves across cluster memberships and edge cluster references.");
        ...
        Assert(api.Contains("--cfx-node-fill", StringComparison.Ordinal) && api.Contains("--cfx-node-stroke", StringComparison.Ordinal) && api.Contains("cfx-graph-node-shadow", StringComparison.Ordinal) && !api.Contains("style.shadow === true ? 'filter:drop-shadow", StringComparison.Ordinal), "Browser graph patches should apply node colors to marks and shadows to complete node groups so SVG filters cannot clip individual card artwork.");
        ...
        Assert(validation.Contains("detachGraphPatchRemovedClusters", StringComparison.Ordinal) && validation.Contains("setGraphAttribute(node, 'data-node-cluster', null)", StringComparison.Ordinal), "Browser graph patches should mirror core atomic removal semantics by clearing node-side references to removed clusters.");
        ...
        Assert(api.Contains("graphPatchVirtualElement(root, 'graph-node', 'cfx-graph-node')", StringComparison.Ordinal) && api.Contains("graphPatchVirtualElement(root, 'graph-edge', 'cfx-graph-edge')", StringComparison.Ordinal), "Accelerated graph patches should keep new nodes and edges in the virtual document so SVG runtime overlays do not duplicate persistent base marks.");
