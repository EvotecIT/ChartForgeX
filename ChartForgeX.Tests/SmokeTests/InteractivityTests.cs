...
internal static partial class SmokeTests {
    private static void InteractivityContractsAreHostNeutral() {
        ...
        var renderedTargets = new ChartInteractionScenario { Id = "rendered-targets", Label = "Rendered targets" }
            .AddRegionStep("region-emea")
            .AddNodeStep("service-api")
            .AddLinkStep("api-database")
            .AddLegendStep("latency");
        Assert(renderedTargets.Steps[0].TargetKind == ChartInteractionTargetKinds.Region && renderedTargets.Steps[1].TargetKind == ChartInteractionTargetKinds.Node && renderedTargets.Steps[2].TargetKind == ChartInteractionTargetKinds.Link && renderedTargets.Steps[3].TargetKind == ChartInteractionTargetKinds.Legend, "Scenario helpers should address rendered regions, nodes, links, and legend entries without adapter-specific role strings.");
    ...
    private static void InteractiveHtmlWrapsSvgWithoutExternalDependencies() {
        ...
        Assert(html.Contains("renderedTargetKind(node) === kind && renderedTargetId(node, kind) === target", StringComparison.Ordinal) && html.Contains("kind === 'point' && data.cfxPoint === target", StringComparison.Ordinal), "Interactive HTML should prefer standardized rendered-target identities while preserving local point-ordinal compatibility.");
        ...
        Assert(html.Contains("const renderedTargetSelector = '.cfx-interactive-region,[data-cfx-label],[data-cfx-series]", StringComparison.Ordinal) && html.Contains("querySelectorAll(renderedTargetSelector)).filter(isInteractiveTarget)", StringComparison.Ordinal), "Rendered-target normalization should use the semantic mark selector and the same nested-target filter as event binding.");
        Assert(!html.Contains("const renderedTargetSelector = '[data-cfx-label],[data-cfx-role],[data-cfx-series]", StringComparison.Ordinal), "Rendered-target normalization should not promote every structural data-cfx-role element into an interactive target.");
        Assert(html.Contains("const roleKinds = new Set((route ? route.steps : [])", StringComparison.Ordinal) && html.Contains("roleKinds.has((node.dataset || {}).cfxRole || '')", StringComparison.Ordinal), "Scenario discovery should preserve explicit AddRoleStep targets without promoting every structural role into the interactive target contract.");
        ...
        Assert(html.Contains("const applyRenderedTargetContract = (root)", StringComparison.Ordinal) && html.Contains("data-cfx-target-kind", StringComparison.Ordinal) && html.Contains("targetId: renderedTargetId(node, targetKind)", StringComparison.Ordinal), "Interactive charts should normalize series, point, annotation, region, node, link, and legend targets before binding interactions.");
        ...
        Assert(html.Contains("const scenarioRevealTargets = (route, stepIndex, targets)", StringComparison.Ordinal) &&
               html.Contains("if (step && (step.targetKind || '') === 'series') return []", StringComparison.Ordinal) &&
               html.Contains("revealNodes(root, scenarioRevealTargets(route, index, targets), emit, sync, 'scenario-step')", StringComparison.Ordinal),
            "Interactive scenario playback should reveal precise route targets without covering series-wide steps in duplicate mark labels.");
        Assert(html.Contains("scenarioTargetCandidates(root, route)", StringComparison.Ordinal) && html.Contains("visualKinds.has(renderedTargetKind(node))", StringComparison.Ordinal) && html.Contains("node.querySelector('.cfx-interactive-region", StringComparison.Ordinal), "Interactive scenarios should mute rendered data targets without dimming titles, axes, or parent SVG containers.");
