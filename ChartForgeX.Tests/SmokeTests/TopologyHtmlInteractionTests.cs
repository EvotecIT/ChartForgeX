...
internal static partial class SmokeTests {
    ...
    private static void TopologyHtmlPagesExposeSelectionInteractions() {
        ...
        Assert(viewportHtml.Contains("if (!(event.target instanceof Element && event.target.closest(selectables))) event.preventDefault();", StringComparison.Ordinal), "Topology dragging should suppress background selection while preserving native focus for nodes, groups, and edges.");
