...
internal static partial class SmokeTests {
    ...
    private static void TopologyHtmlPagesExposeSelectionInteractions() {
        ...
        Assert(viewportHtml.Contains("if (isViewportChrome(event.target)) return;\n        if (!(event.target instanceof Element && event.target.closest(selectables))) event.preventDefault();\n        const state = viewportState();", StringComparison.Ordinal), "Topology dragging should suppress background selection while preserving native focus for nodes, groups, and edges.");
