...
public sealed class TopologyMeasuredTypographyTests {
    ...
    public void InstalledArialUsesRequestedFaceAndReservesNativeBoldAdvances() {
        ...
        Assert.True(new TextMeasurementContext(theme.FontFamily).Measure(text, 34, true) >= nativeWidth);
    }
    ...
    public void SubtitleChipsContainTheirMeasuredText(TopologyNodeDisplayMode mode) {
        ...
        var options = new TopologyRenderOptions { NodeDisplayMode = mode, CardSubtitleMode = TopologyCardSubtitleMode.Chip, IncludeTileSubtitles = true };
        ...
        Assert.True(new TextMeasurementContext(theme.FontFamily).Measure(text, 9.5, true) <= width - 17.9);
    ...
    public void WidthTrimmingFitsAndRetainsTextElementBoundaries(string input, double width) {
        ...
        var context = new TextMeasurementContext(style.Font.Family);
